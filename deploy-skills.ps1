...
Write-Host "5) All Individual Skills Combined (313 skills)"
...
switch ($choice) {
    ...
    "3" {
        ...
        $eccSkills = $allSkills | Where-Object { $_ -notin $anthropicSkills -and $_ -notin $superpowersSkills -and $_ -ne 'clipify' -and $_ -ne 'video-use' -and $_ -ne 'youtube-clipper' -and $_ -ne 'remotion-best-practices' -and $_ -ne 'video-copywriting' -and $_ -ne 'video-ui-ux-design' -and $_ -ne 'video-producer' -and $_ -ne 'promo-video' -and $_ -ne 'skill-loader' -and -not $_.StartsWith('godot-') }
