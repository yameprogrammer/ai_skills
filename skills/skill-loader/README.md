...
### 2. 🌌 Google Antigravity (CLI & Desktop)
안티그래비티 역시 커스텀 스킬 및 룰 셋 기능을 지원합니다.
*   **전역 설치 방법**:
    안티그래비티의 사용자 스킬 저장 디렉토리인 `C:\Users\parkp\.gemini\antigravity-cli\skills\` 하위에 `skill-loader` 폴더를 생성하고, `SKILL.md` 파일을 복사해 넣습니다.
*   **작동**:
    안티그래비티 CLI(`agy`) 또는 데스크톱 2.0 앱이 구동될 때 자동으로 전역 스킬을 인지하여 동작하므로, 유저가 연동하는 모든 대화 세션에서 하이브리드 스킬 로더가 상시 백그라운드 가동됩니다.
